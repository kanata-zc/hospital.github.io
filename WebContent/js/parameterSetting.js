				...
				if(list.length>0){
					for(var i=0;i<list.length;i++){
						...
						var $tr = "<tr onclick='showParameter(this);'><td><label style='display:none;'>"+parameter.id+
							  "</label>"+
							  "<input readonly='readonly' size='42.9px'  type='text' value='"+parameter.code+
							  "'/></td><td>"+
							  "<input readonly='readonly' size='42.9px'  type='text' value='"+parameter.name+
							  "'/></td></tr>";
						$tbody.append($tr);
					}
				}
				...
				if(list.length>0){
					for(var i=0;i<list.length;i++){
						...
						var $tr = "<tr><td><label style='display:none;'>"+paracode.id+
						  "</label>"+
						  "<input readonly='readonly' size='41px' type='text' value='"+paracode.code+
						  "'/></td><td>"+
						  "<input  size='41px' type='text' value='"+paracode.name+
...
function codeadd(){
	...
	"<input onblur='checkNo(this);' onfocus='checkNof(this);' size='41px' type='text'"+ 
	"</td><td>"+
	"<input  size='41px' onblur='checkName(this);' onfocus='checkNamef(this);' type='text'"+
