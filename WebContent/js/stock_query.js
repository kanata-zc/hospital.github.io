...
function showList(list,start,end){
	if(list.length>0){
		for(var i=start;i<end;i++){
			var stock = list[i];
			var $tr = "<tr>"+
				"<td class='num'><a href='javascript:void()' onclick='jump(this);'>"+(i+1)+"</a></td>"+
...
function jump(obj){
	var drugsId = $(obj).parent().siblings().html();
	var drugsName = $(obj).parent().next().next().html();
	var price = $(obj).parent().next().next().next().html();
