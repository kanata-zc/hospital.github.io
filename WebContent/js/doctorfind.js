...
function select(){
	...
	var data = {"name":name,"title":title,"startTime":startTime,"endTime":endTime,"department":department,"state":state};
	//执行查询
	$.ajax({
...
function ok(){
	...
	if(checkName()){
		...
		$.ajax({
			...
			data:{"id":id,"name":name,"keshi":keshi,"gender":gender,"zhicheng":zhicheng},
			type:"post",
			dataType:"JSON",
			success:function(result){
				if(result.state == 0){
