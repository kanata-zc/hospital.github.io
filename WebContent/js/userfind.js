	...
	if(list.length>0){
		for(var i=start;i<end;i++){
			...
			var $tr = $(trStyle+
				        ...
				        "<td class='node'>"+describe+"<span style='display:none'>"+user.describe+"</span></td>"+
...
function userUpdate(obj){
	...
	//得到所选用户的权限
	var quanxian  = $(obj).parent().parent().children().eq(3).find("span").text();
...
function ok(){
	...
	if(checkName()&&checkPhone()){
		...
		$.ajax({
			...
			data:{"id":id,"name":name,"phone":phone,"state":state},
			type:"post",
			dataType:"JSON",
			success:function(result){
				if(result.state == 0){
