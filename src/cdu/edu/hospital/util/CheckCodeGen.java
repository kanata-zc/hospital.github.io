...
public class CheckCodeGen implements Serializable {
	...
	private static final long serialVersionUID = -7747429507373323511L;
	...
	public boolean verifyCode(String codeStr, HttpServletRequest request, boolean clear) {
		...
		if (clear) // 清除验证
			cleanCode(session);
		if (code == null || !code.enable || StringUtils.isBlank(code.code) || !code.code.equalsIgnoreCase(codeStr.trim())) {
			return false;
		}
		// 有效
